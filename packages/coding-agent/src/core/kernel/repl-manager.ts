...
export class ReplKernelManager {
	 ...
	 * bytes; checking rotation once per spawn is enough.
	 */
	...
	private stderrTail(): string {
		...
		if (this.options.stderrLogPath) {
			...
			try {
				descriptor = openSync(this.options.stderrLogPath, "r");
				const size = fstatSync(descriptor).size;
				const length = Math.min(1024, size);
				const tail = Buffer.allocUnsafe(length);
				if (length > 0) readSync(descriptor, tail, 0, length, size - length);
				fileTail = tail.toString("utf8");
			} catch {
