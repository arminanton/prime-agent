...
export class ModelRegistry {
	...
	private loadBuiltInModels(
		...
		// When a specific GitHub account/host is pinned for Copilot, the resolved
		// token is a raw gh token with no embedded proxy-ep, so the per-plan host
		// (api.individual.githubcopilot.com) rejects it with 421. Route Copilot to
		// the shared front door (or enterprise Copilot host) that accepts it.
		const copilotBaseUrl = copilotPinnedBaseUrl();
		return mergePrimeInferenceModels(bundledModels, livePrimeInferenceModels)
			.filter((model) => this.isCopilotModelSelectable(model))
			.map((model) => {
				...
				if (model.provider === "github-copilot") {
					if (copilotBaseUrl) {
						configuredModel = { ...configuredModel, baseUrl: copilotBaseUrl };
					}
					// Correct the api mode from the account's live supported_endpoints when it
					// disagrees with the baked routing (responses-only grok/mai-code/oswe 400 on
					// /chat/completions). The baked compat block is api-family-specific, so drop
					// it when the family changes.
					const liveApi = this.copilotApiById.get(model.id);
	...
	/**
	 * Filter Copilot to the account's live entitlements when known, so an
	 * unentitled model can never be selected (which would 400). Before the first
	 * successful fetch, keep the fallback catalog while withholding candidates
	 * known to require positive live entitlement.
	 */
	private isCopilotModelSelectable(model: Model<Api>): boolean {
		if (model.provider !== "github-copilot") return true;
		if (this.copilotEntitlementKnown) return this.copilotEntitledModelIds.has(model.id);
		return !COPILOT_LIVE_CATALOG_REQUIRED_MODEL_IDS.has(model.id);
	}
	...
	private writeCopilotEntitlementCache(cache: CopilotEntitlementCache): void {
		...
		try {
			writeFileAtomicSync(cachePath, JSON.stringify(cache), { mode: 0o600 });
		} catch {
